...
> → **Request a public certificate** → Entrer votre domaine (ex. `ecommerce.mondomaine.app`)  
...
| `BACKEND_HOST` | `ecommerce.mondomaine.app` |
...
**Résultat :** `https://ecommerce.mondomaine.app` route ~66% vers EC2/Beanstalk et ~33% vers ECS Fargate (selon les poids), avec stickiness pour garder chaque visiteur sur la même plateforme.
...
1. Ouvrir `https://ecommerce.mondomaine.app` → Page d'accueil du shop ✅
...
| ACM | Certificat SSL (eu-west-1) | `ecommerce.mondomaine.app` |
| Cloudflare | DNS CNAME | `ecommerce.mondomaine.app` → ALB public |
