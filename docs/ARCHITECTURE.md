...
3. [Couche DNS & TLS](#3-couche-dns--tls)
...
ALB public (HTTPS :443, ACM)  ──  forward pondéré + stickiness
    │
    ├── Option A : EC2 (NGINX natif, instance unique)
    ├── Option B : Elastic Beanstalk (Docker/ECR)
    └── Option C : ECS Fargate (Express Mode)
                │ (les 3 servent le même frontend React + NGINX)
                ▼
         ALB interne EKS (AWS Load Balancer Controller)
                │
    ┌───────────┼─────────────┬────────────┐
    ▼           ▼             ▼            ▼
auth:3001  product:3002  order:3003  review:3004
    │           │             │            │
    └───────────┴─────────────┴────────────┘   EKS Auto Mode + Helm (HPA)
                       │ MySQL :3306
                 RDS MySQL 8.0
                  (ecommerce_db)
```
...
|-------------|----------------|------|
| IP fixe `192.168.56.115` (MariaDB) | RDS MySQL endpoint DNS | Endpoint stable géré par AWS (Multi-AZ activable pour le failover) |
| IP fixe `192.168.56.111` (K8s NodePort 30080) | ALB interne EKS | Créé automatiquement par AWS LB Controller |
| IP fixe `192.168.56.114` (Frontend VM) | ALB public + EC2/Beanstalk/ECS | Remplacé par une couche managée |
...
└──────────────────────────────────────┬───────────────────────────────────────┘
                         ...
                         ┌─────────────┼─────────────┐
                         ▼             ▼             ▼
                   Cloudflare    [CloudFront]        ACM
                    (DNS)        (optionnel)      (SSL/TLS)
                         └─────────────┬─────────────┘
                                       │ HTTPS :443
┌──────────────────────────────────────────────────────────────────────────────┐
...
│                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────┐   │
│  │ Subnets publics - 10.0.0-32.0/20 (eu-west-1a/b/c)                     │   │
│  │                                                                       │   │
...
│  │  │   EC2    │  │Beanstalk │  │   ECS    │                             │   │
│  │  │ instance │  │  (PaaS)  │  │ Fargate  │  NGINX + React build        │   │
│  │  │  Option  │  │ Option B │  │ Option C │  proxy /api → ALB interne   │   │
│  │  │    A     │  │  Docker  │  │ Express  │                             │   │
│  │  └──────────┘  └──────────┘  └──────────┘                             │   │
│  └───────────────────────────────────────────────────────────────────────┘   │
│                                      │                                       │
│                                      ▼ HTTP                                  │
│  ┌───────────────────────────────────────────────────────────────────────┐   │
│  │  Subnets privés - 10.0.128-160.0/20 (eu-west-1a/b/c)                  │   │
│  │                                                                       │   │
│  │        ALB interne - Ingress EKS (AWS LB Controller)                  │   │
│  │                         │                                             │   │
│  │  ┌──────────────────────────────────────────────────────────────────┐ │   │
│  │  │  EKS - ecommerce-cluster (Auto Mode)                             │ │   │
│  │  │  Nodes Bottlerocket gérés automatiquement par AWS · HPA          │ │   │
│  │  │                                                                  │ │   │
│  │  │  namespace: ecommerce                                            │ │   │
│  │  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ │ │   │
...
│  │  │  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘ │ │   │
│  │  └──────────────────────────────────────────────────────────────────┘ │   │
│  │       SG nœuds EKS (auto) : :3001-3004 ← ALB interne · self           │   │
│  └───────────────────────────────────────────────────────────────────────┘   │
│                                     │ MySQL :3306                            │
│  ┌───────────────────────────────────────────────────────────────────────┐   │
│  │  Subnets DB - 10.0.48-80.0/20 (eu-west-1a/b/c)                        │   │
│  │                                                                       │   │
│  │    ┌────────────────────────────────────────┐                         │   │
...
**Subnets publics** - exposés à internet via l'Internet Gateway. Hébergent l'ALB public, la NAT Gateway, et les instances frontend (EC2, Beanstalk, ECS tasks).

| Nom | CIDR | AZ | Ressources |
|-----|------|----|-----------|
| ecommerce-subnet-public1-eu-west-1a | 10.0.0.0/20 | eu-west-1a | ALB public, NAT Gateway, frontend |
| ecommerce-subnet-public2-eu-west-1b | 10.0.16.0/20 | eu-west-1b | ALB public, frontend |
| ecommerce-subnet-public3-eu-west-1c | 10.0.32.0/20 | eu-west-1c | ALB public, frontend |

> ℹ️ Les instances frontend doivent activer **Auto-assign public IP** au lancement (l'option `MapPublicIpOnLaunch` est à `false` par défaut sur ces subnets, malgré la route Internet Gateway).

**Subnets privés** - pas d'accès internet entrant. Les nœuds EKS sortent vers internet via la NAT Gateway (pull d'images, APIs AWS).

| Nom | CIDR | AZ | Ressources |
|-----|------|----|-----------|
| ecommerce-subnet-private1-eu-west-1a | 10.0.128.0/20 | eu-west-1a | Nœuds EKS, pods, ALB interne |
| ecommerce-subnet-private2-eu-west-1b | 10.0.144.0/20 | eu-west-1b | Nœuds EKS, pods, ALB interne |
| ecommerce-subnet-private3-eu-west-1c | 10.0.160.0/20 | eu-west-1c | Nœuds EKS, pods, ALB interne |

Tags Kubernetes obligatoires sur ces subnets :
- `kubernetes.io/role/internal-elb = 1` → l'ALB interne EKS est créé ici
- `kubernetes.io/cluster/ecommerce-cluster = shared` → découverte par le LBC

**Subnets base de données** - isolés, sans route vers internet. Seule la communication via le Security Group depuis les nœuds EKS est autorisée.

| Nom | CIDR | AZ | Ressources |
|-----|------|----|-----------|
| ecommerce-db-a | 10.0.48.0/20 | eu-west-1a | RDS MySQL |
| ecommerce-db-b | 10.0.64.0/20 | eu-west-1b | Subnet group (standby potentiel) |
| ecommerce-db-c | 10.0.80.0/20 | eu-west-1c | Subnet group |
...
> ℹ️ Le frontend ne contacte **pas directement** les nœuds EKS. Il passe par l'**ALB interne EKS** (créé par l'AWS Load Balancer Controller), qui route ensuite vers les nœuds sur les ports microservices.

```
Internet (0.0.0.0/0)
    │ :80 :443
    ▼
ecommerce-sg-alb              (ALB public)
    │ :80 ← ecommerce-sg-alb
    ▼
ecommerce-sg-frontend         (EC2 / Beanstalk / ECS)
    │ :80 ← ecommerce-sg-frontend
    ▼
ecommerce-sg-alb-interne-backend   (ALB interne EKS)
    │ :3001-3004 ← ecommerce-sg-alb-interne-backend   [géré par le LBC]
    ▼
ecommerce-sg-eks-nodes        (nœuds EKS)
    │ :3306 ← ecommerce-sg-eks-nodes
    ▼
ecommerce-sg-rds              (RDS MySQL)
```
...
| Direction | Protocol | Port | Source |
|-----------|----------|------|--------|
| Inbound | TCP | 3001-3004 | `ecommerce-sg-alb-interne-backend` (règle `targetGroupBinding` du LBC) |
| Inbound | All | All | self (communication intra-cluster, EFA) |
...
| Direction | Protocol | Port | Source |
|-----------|----------|------|--------|
| Inbound | TCP | 3306 | `ecommerce-sg-eks-nodes` |
| Inbound | TCP | 3306 | `ecommerce-sg-eks-controlplane` |
...
Le même frontend React (build `dist/` servi par NGINX, proxy `/api/*` vers l'ALB interne EKS) est déployé de **3 façons** :

- **Option A (EC2)** : NGINX **natif** sur la VM, build fait directement (`npm run build`), **sans Docker**.
- **Option B (Beanstalk)** & **Option C (ECS)** : **image Docker** `nginx:stable-alpine` (build React + NGINX), hébergée sur ECR. La variable `BACKEND_URL` est injectée par `envsubst` au démarrage du conteneur.

Le badge plateforme dans la navbar (`VITE_DEPLOY_PLATFORM`, build-time) indique sur quelle option tourne l'instance servie.
...
- **Enregistrement manuel** dans le Target Group `ecommerce-tg-frontend` (type Instance, port 80)
- **NGINX** : sert le build React (`dist/`) et proxifie `/api/*` vers l'ALB interne EKS

> ℹ️ Choix portfolio : **instance unique** (pas d'Auto Scaling Group) pour rester en Free Tier et simplifier. Un ASG s'ajouterait facilement via un Launch Template pour la HA en production.
...
- **Environment** `ecommerce-frontend-prod` : 1 instance EC2 (`t3.micro`) gérée par Beanstalk
- **Dockerrun.aws.json** : déclare l'image ECR (`ecommerce/frontend:latest`), le port 80, et les variables d'env

**Configuration `Dockerrun.aws.json` :**
```json
{
  "AWSEBDockerrunVersion": "1",
  "Image": { "Name": "ACCOUNT.dkr.ecr.eu-west-1.amazonaws.com/ecommerce/frontend:latest", "Update": "true" },
...
**Avantages :** déploiement en une commande (upload du `Dockerrun.aws.json`), gestion OS/patches automatique, health checks et logs CloudWatch intégrés
...
**Composants (déployés via ECS Express Mode) :**
- **Cluster ECS** : cluster `default` Fargate (On-Demand, 0.25 vCPU / 0.5 GB par task)
- **Task Definition** : blueprint du conteneur (image ECR `:ecs`, CPU/mémoire, port 80, variables d'env, logs CloudWatch). Chaque révision est immutable
- **Service ECS** : maintient 1-2 tasks (auto scaling CPU 70%), enregistrées dans le Target Group IP `ecs-gateway-tg`, lui-même attaché à l'ALB public partagé
...
| Critère | EC2 | Beanstalk | ECS Fargate |
|---------|-----|-----------|-------------|
...
| Debugging | SSM Session Manager | SSM / EB logs | ECS Exec |
| Coût charge stable | Le moins cher | = EC2 | Le plus cher |
...
*Listener HTTPS :443* → **Forward pondéré vers 2 Target Groups** (ACM + SSL Policy `ELBSecurityPolicy-TLS13-1-2-2021-06`) :

| Target Group | Type | Sert | Poids |
|--------------|------|------|-------|
| `ecommerce-tg-frontend` | instance | EC2 + Beanstalk | 2 |
| `ecs-gateway-tg-xxxx` | ip | ECS Fargate (auto-créé par Express Mode) | 1 |

Health check : `GET /` (le frontend NGINX répond 200 sur `/`).

### Sticky sessions

**Activées** (LB cookie + group stickiness sur la règle). Indispensable ici car chaque plateforme sert un **build React au hash différent** : sans stickiness, un visiteur basculerait d'un TG à l'autre à chaque requête et chargerait un `index.html` d'un build mais un bundle JS d'un autre → page blanche (`NS_ERROR_CORRUPTED_CONTENT`). La stickiness garde chaque visiteur sur le même Target Group (donc la même plateforme/build).
...
| Mode | **EKS Auto Mode** (nodes gérés par AWS) |
| Endpoint access | Public + Private |
| Subnets | Privés eu-west-1a/b/c |
| Add-ons | CoreDNS · kube-proxy · Amazon VPC CNI · Metrics Server · AWS Secrets & Config Provider (ASCP) |
...
| Paramètre | Valeur |
|-----------|--------|
| Provisioning | **Automatique** (AWS gère type, scaling, patching) |
| AMI | Bottlerocket (EKS Auto, Standard) |
...
**Conséquence :** chaque nœud peut héberger autant de pods qu'il a d'IP secondaires disponibles. EKS Auto Mode choisit automatiquement la taille des nœuds en fonction de la densité de pods requise.
...
|---------|-------------|-------------|-------------|
| auth-service | 1 | 3 | 70% |
| product-service | 1 | 3 | 70% |
| order-service | 1 | 3 | 70% |
| review-service | 1 | 2 | 70% |

Le HPA interagit avec **EKS Auto Mode** : si les pods pending ne peuvent pas être schedulés (nœuds pleins), Auto Mode provisionne automatiquement un nœud supplémentaire (~1-2 minutes).
...
**Note sur le fulltext search :** La table `products` a un index `FULLTEXT` sur `(name, description)` - la route `/search?q=laptop` utilise `MATCH(name, description) AGAINST(? IN BOOLEAN MODE)`. Compatible MySQL 8.0.
...
### Configuration RDS (réelle)

| Paramètre | Valeur |
|-----------|--------|
| Engine | MySQL 8.4.8 |
| Instance class | db.t4g.micro (ARM Graviton) |
| AZ | eu-west-1b (single-AZ) |
| Multi-AZ | Non (Free Tier / portfolio) |
| Chiffrement | AES-256 (at rest) |
| SSL en transit | Oui |
| Backups automatiques | 1 jour de rétention |
...
Les nœuds EKS Auto Mode utilisent le rôle `AmazonEKSAutoNodeRole` (géré par AWS) qui inclut l'accès registry. Note : les microservices sont sur **GHCR public** (pas ECR), donc aucun credential de pull n'est requis pour eux.
...
```json
{ "DB_USER": "devops_user", "DB_PASSWORD": "••••••••" }
```
...
| Rôle | Créé par | Usage |
|------|----------|-------|
| `AmazonEKSAutoClusterRole` | EKS Auto Mode | Control plane du cluster |
| `AmazonEKSAutoNodeRole` | EKS Auto Mode | Nœuds gérés automatiquement |
| `AWSServiceRoleForAmazonEKS` | AWS (service-linked) | Opérations EKS |
| `ecommerce-eks-secrets-role` | Vous (IRSA) | Pods → AWS Secrets Manager via OIDC |
| `AWSLoadBalancerControllerRole` | Vous (IRSA) | LBC → créer/modifier ALB + Target Groups |
| `ecsTaskExecutionRole` | ECS | Pull image ECR + logs CloudWatch (tasks Fargate) |
| `ecsInfrastructureRoleForExpressServices` | ECS Express | Provisioning de l'infra ECS |
| `aws-elasticbeanstalk-ec2-role` | Beanstalk | Instances EB : ECR + SSM (Option B) |
| `ecommerce-frontend-ec2-role` | Vous | Instance EC2 frontend : SSM uniquement (Option A) |
...
| `/aws/rds/instance/ecommerce-mysql` | RDS slow queries | 7 jours |
...
2. ALB public (ecommerce-alb-pub)
   → Listener HTTPS :443 (terminaison TLS via ACM)
   → Forward pondéré + stickiness → ecommerce-tg-frontend OU ecs-gateway-tg
   → Choisit une instance frontend healthy

3. Instance frontend (EC2 / Beanstalk / ECS)
   → NGINX reçoit GET /products
   → Sert le fichier dist/index.html (SPA React)
   → Le navigateur charge la page

4. React (dans le navigateur) → fetch('/api/products')
   → NGINX reçoit GET /api/products
   → proxy_pass vers http://ALB_INTERNE_EKS/api/products

5. ALB interne EKS
   → Route /api/products vers le Target Group product-service (type IP)
   → Choisit un pod healthy (port 3002)

6. Pod product-service (:3002)
   → Reçoit GET /api/products
   → Pas d'auth requise (route publique)
   → mysql2 → connexion pool vers endpoint RDS MySQL

7. RDS MySQL
   → SELECT * FROM products ORDER BY created_at DESC
   → Retourne les résultats

8. Remontée des réponses dans l'ordre inverse
   RDS MySQL → product-service → ALB interne → NGINX → ALB public → Navigateur
```
...
Ensuite, la simplicité opérationnelle : une seule instance RDS MySQL à gérer, sauvegarder, et monitorer. Pour un portfolio DevOps, ce choix permet de se concentrer sur l'orchestration Kubernetes plutôt que sur la complexité des transactions distribuées.
...
| EKS Nodes (Auto Mode) | ~2 nœuds gérés | ~$30-60 |
| RDS MySQL | db.t4g.micro | ~$15 |
| NAT Gateway | 1 × ($0.045/h × 730h) | ~$33 |
| ALB public | ~$16 + $0.008/LCU | ~$16 |
| ECR | 1 repo frontend | ~$1 |
| Secrets Manager | 2 secrets | ~$1 |
| Cloudflare DNS | gratuit | $0 |
...
| Mode | Coût/mois |
|------|----------|
| Infrastructure de base (EKS + RDS + ALB + NAT) | ~$170-200 |
| + Option A : EC2 (1 × t3.micro, Free Tier) | +$0-7.5 |
| + Option B : Beanstalk (1 × t3.micro) | +$7.5 |
| + Option C : ECS Fargate (0.25 vCPU, 0.5 GB) | +$9 |
