...
4. [RDS Aurora — Base de données](#4-rds-aurora--base-de-données)
...
| NAT gateways | **In 1 AZ** (économique) ou **1 per AZ** (production) |
| VPC endpoints | None |
...
|------|----------|------|--------|
| MySQL/Aurora | TCP | `3306` | `ecommerce-sg-eks` |

Cliquer **Create security group**

---

## 4. RDS Aurora — Base de données

Aurora MySQL est compatible MariaDB 10.11 — votre schéma `ecommerce_db.sql` et vos drivers `mysql2` fonctionnent sans modification.

### 4.1 Créer le Subnet Group RDS

**Navigation :** Barre de recherche → `RDS` → **RDS**

**Dans le menu gauche :** **Subnet groups** → **Create DB subnet group**

| Champ | Valeur |
|-------|--------|
| Name | `ecommerce-db-subnet-group` |
| Description | `Subnets pour RDS Aurora ecommerce` |
| VPC | `ecommerce-vpc` |
| Availability Zones | Sélectionner `eu-west-1a` ET `eu-west-1b` |
| Subnets | Sélectionner `ecommerce-db-a` ET `ecommerce-db-b` |

Cliquer **Create**

---

### 4.2 Créer le cluster Aurora

**Navigation :** RDS → **Databases** → **Create database**

**Étape par étape dans le formulaire :**
...
- Version : **Aurora MySQL 3.05.2 (compatible with MySQL 8.0.32)**

> 💡 MySQL 8.0 est pleinement compatible avec vos drivers `mysql2` Node.js et votre schéma MariaDB 10.11.

**Templates :**
- ✅ **Production** (active le multi-AZ et les sauvegardes)

**Settings :**

| Champ | Valeur |
|-------|--------|
| DB cluster identifier | `ecommerce-aurora-cluster` |
...
| ✅ Enable Enhanced monitoring | Coché |

Cliquer **Create database** → Attendre **~8 minutes**

**Récupérer l'endpoint :**  
Une fois le statut `Available`, cliquer sur `ecommerce-aurora-cluster` → Section **Connectivity & security** → Copier le **Writer endpoint** (ex. `ecommerce-aurora-cluster.cluster-xxxx.eu-west-1.rds.amazonaws.com`).
...
mysql -h ecommerce-aurora-cluster.cluster-xxxx.eu-west-1.rds.amazonaws.com \
...
- Database : sélectionner `ecommerce-aurora-cluster`
...
kubectl create secret generic ecommerce-secrets \
  --namespace ecommerce \
  --from-literal=DB_HOST="<endpoint_aurora_copié_étape_4>" \
...
### 13.2 RDS — Vérifier la connexion

**Navigation :** RDS → **Databases** → `ecommerce-aurora-cluster`
...
**Widget 1 — RDS connexions :**
- Type : Line
- Metrics : RDS → Per-Database Metrics → `ecommerce-aurora-cluster` → DatabaseConnections
...
| RDS | Cluster Aurora | `ecommerce-aurora-cluster` |
...
1. Metric → RDS → Per-Database Metrics → `ecommerce-aurora-cluster` → `CPUUtilization`
